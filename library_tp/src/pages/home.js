...
function Home() {
  ...
  const ReturnBook = async (book_id) => {
    const user_id = JSON.parse(localStorage.getItem("account")).id;
    ...
    if (bookData.length > 0) {
      const newStock = bookData[0].num_in_stock + 1;
      await supabase
          .from("books")
          .update({num_in_stock: newStock})
          .eq("id", book_id);
    }

    // Обновляем список книг пользователя
    setUserBooks((prevBooks) =>
        prevBooks.filter((book) => book.id !== book_id)
    );
  };
        ...
        let book_ids = [];
        {
          userBooks?.map((book) => (
              <li className="book-item" key={book.id}>
                {book.title}
                <button className="return-book-button" onClick={() => ReturnBook(book.id)}>
                  Вернуть книгу
                </button>
              </li>
          ))
          ...
          <div className="librarian-content">
            ...
            <div className="users-section">
              ...
              <div className="user-list">
                <ul>
                  {users
                      ?.filter((user) => user.role === "customer") // Добавляем фильтрацию по роли
                      .map((user) => (
                            ...
                            {user.id !== JSON.parse(localStorage.getItem("account")).id ? (
                                <div className="user-action">
                                  ...
                                  <ul className="user-books-list">
                                    {eachuserBooks[user.id]?.map((userBook) => {
                                      const book = books.find((b) => b.id === userBook.book_id);
                                      return (
                                          <li key={userBook.book_id} className="user-book-item">
                                            {book ? book.title : "Неизвестная книга"}
                                            <button
                                                className="return-book-button"
                                                onClick={() => ReturnBook(userBook.book_id)}
                                            >
                                              Вернуть книгу
                                            </button>
                                          </li>
